...
def get_from_github(device):
        print("Going to fetch %s from bass github" % device)
        try:
        ...
        githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:Bass-ROM+in:name+fork:true" % device)
        if githubauth:
        ...
        for res in result['items']:
            if (res['name'].startswith("device_") and res['name'].endswith("_%s" % device)):
                ...
                devicetree = res['full_name'].replace("Bass-ROM/", "")
                if add_to_local_manifest(devicepath, devicetree, "bass"):
                    reposync(devicetree)
                break

def checkdeps(repo_path):
    bassdeps = glob.glob(repo_path + "/bass.dependencies")
    if ((len(bassdeps)) < 1):
        ran_checkdeps_on.append("NO_DEPS:\t\t" + repo_path)
        return
    else:
        if (len(bassdeps) > 0):
            ran_checkdeps_on.append("HAS_BASS_DEPS:\t" + repo_path)
            bassdeps = bassdeps[0]
            bassdeps = open(bassdeps, 'r')
            bassdeps = json.loads(bassdeps.read())
            for dep in bassdeps:
                try:
                    branch = dep['branch']
                except:
